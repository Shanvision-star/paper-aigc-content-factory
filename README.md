...
| `voiceover-emotion-coach` | 生成可复用教学感口播 `delivery_style` 和引擎情绪提示 |
...
| `voiceover-emotion-coach` | Creates reusable teaching-style `delivery_style` and engine emotion prompts |
...
口播可以更绘声绘色，但表达力必须是可审核的元数据，不是隐藏台词。`voiceover-emotion-coach` 在 `spoken_text` 锁定后、TTS 小样前运行，输出跨论文复用的 `delivery_style` 和引擎级 `engine_emotion_prompt`。

强约束：

- 默认目标是 teaching-style prosody：像一位耐心老师在给聪明但刚入门的学生讲论文。
- `delivery_style` 描述角色、语气、语速、能量、重点句、公式慢读和英文术语清晰度。
- IndexTTS2 可使用 `use_emo_text=true`、`emo_text`、`emo_alpha`、`use_random=false` 这类引擎参数；CosyVoice 等备选引擎使用各自 instruction/prompt 字段。
