...
- 公式标准化：公式必须完整、清晰、可标注，可用论文截图、公式软件截图、KaTeX/MathJax/SVG、Manim 或 source-backed MATLAB 导出资产；MATLAB 资产必须遵守 `docs/visual_system/MATLAB.md`。
...
| `visual-orchestrator` | 分配 SVG、Manim、MATLAB、HyperFrames、图表等视觉引擎 |
...
- Formula standardization: formulas must be complete, sharp, and annotatable via paper crops, formula-editor screenshots, KaTeX/MathJax/SVG, Manim, or source-backed MATLAB exports governed by `docs/visual_system/MATLAB.md`.
...
| `visual-orchestrator` | Assigns SVG, Manim, MATLAB, HyperFrames, and chart engines |
...
- Default `npm test` does not call real LLM, TTS, HyperFrames render, Manim, MATLAB rendering, or network services.
...
P0 不调用真实 LLM、TTS、HyperFrames MP4 渲染、Manim、MATLAB 渲染或发布平台。默认验证必须保持确定性和低成本；真实 provider smoke 只能作为显式的独立检查，不进入 `npm test` 基线。
...
`DESIGN.md -> FRAME.md -> MATLAB.md（使用 MATLAB 资产时） -> episode FRAME.md`
...
MATLAB 只作为确定性视觉适配器进入项目，不替代 HyperFrames 最终 composition。完整约束以 `docs/visual_system/MATLAB.md` 为准；后续调用 MATLAB 生成动态图、公式转换或论文机制图时，必须满足：
...
- 真实 MATLAB 渲染不进入默认 `npm test`。默认测试最多检查 manifest、FRAME、脚本入口或 tiny synthetic fixture，不跑完整视频生成。
