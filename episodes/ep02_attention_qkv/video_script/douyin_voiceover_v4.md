...
- Source: user-approved EP02 final voiceover V4.
- Episode connection: directly continues EP01 ending, "下一集，我们把 Q 和 K 的相乘过程拆开..."
- Teaching method: Feynman learning method with formula-first animation.
- TTS rule: keep this as `source_text`; use `script/pronunciation_normalization.md` and `audio/voiceover_audio_prompt.md` for `spoken_text`.
...
模型怎么判断，当前这个 token，应该重点看上下文里的谁？

这里的 token，可以先理解成：模型正在处理的词片段。
...
前面那些 token 的 K 和 V，如果每次都重新算，推理会很慢，显存也会浪费。
...
把上下文关系，变成可以计算、可以加权、可以复用的信息流。

下一集，我们继续拆：

如果一个 QKV 视角已经能看关系，为什么 Transformer 还要一次开很多个 head？
