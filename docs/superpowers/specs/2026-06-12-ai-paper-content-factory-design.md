...
### EP05-Derived Stage Gates

Formula-heavy, TTS-heavy, MATLAB/HyperFrames, or dynamic-subtitle episodes must read and apply `docs/superpowers/specs/2026-06-20-ep05-stage-gate-map.md` before frame lock. The map distributes EP05 RoPE failure modes to every production stage, including source evidence, script/notation contracts, technical review, Ogilvy proof objects, frame specs, MATLAB assets, TTS pronunciation, caption sync, SFX, HyperFrames composition, final MP4 QA, and workflow optimization.

The core rule is: prevent the earliest stage from passing a known EP05-class risk downstream. If a final MP4 review finds a repeated issue, the fix must be routed back to the earliest responsible gate, not only patched in the render layer.
...
- `docs/visual_system/MATLAB.md` 是 MATLAB 视觉适配器规范，约束布局、动画、字体映射、专业术语、公式清晰度、图像重叠检查、manifest、R2021b 优先调用、render environment 记录和 HyperFrames 交接。
...
- Review frames 优先使用 R2021b `exportgraphics` 固定像素导出；除非 episode FRAME 写明理由，不把屏幕窗口状态或裸 `getframe` 当最终帧来源。
- 若 R2021b 超过 180 秒无有效输出、卡住、`-batch` 崩溃或图形启动异常，停止该次尝试并用 R2026a 生成同一资产；PR 审核记录回退原因和两次命令。
- MATLAB 真实导出必须在 manifest 或 PR 中记录活动版本、Windows 文本大小、显示缩放、`rendererinfo`、实际 renderer device、GPU 和最小 figure/export smoke；正式资产默认要求 Windows 辅助功能 `Text size=100%`，例外必须留在 `needs_human_review`。
...
- `90%` 安全区保留 `54px` 左右安全边距、`96px` 上下安全边距；安全区只作为排版约束，不得渲染成黑边、黑底、安全框线或缩小海报。
- 不要为了填满画布让标题、公式或主视觉贴边；背景、纹理和系列视觉必须延展到完整 `1080x1920` 画布。
- 系列封面风格优先参考第四集最终封面 `episodes/ep04_positional_encoding/video_script/cover_ep04_positional_encoding_final_1080x1920_safe90.png` 的浅色纸面、浅灰网格、强标题、论文证据卡和暖橙/蓝色机制强调；不要把非最终版第二集深色封面作为系列参考。
...
- EP05-derived check: model-family examples must be classified as `open_model_evidence`, `product_context`, or `proprietary_unspecified`. Do not turn product context into implementation evidence.
...
- RoPE-like method claims must distinguish paper facts, engineering interpretation, and Feynman analogy. Unsupported claims about accuracy, closed-source model internals, or implementation details block the gate.
...
- 第一幕必须引用已选 `hook_id`。
- Formula-heavy episodes must define `source_text`, `spoken_text`, `caption_text`, and `visual_text` for high-risk notation before TTS or render.
- Avoid mechanically repeating notation such as `n-m`; explain the mechanism in viewer language and map it back to the formula.
- Do not use technically loose phrases such as "returns relative distance" when the reviewed mechanism is a position-related term depending on relative displacement.
...
- MATLAB 生成资产必须遵守 `docs/visual_system/MATLAB.md`，并在 manifest 记录脚本、MATLAB release、render environment、source evidence、canvas、fps 或静态格式、字体映射、术语合同、输出路径、review keyframes 和 HyperFrames 导入后的重叠检查结果。
- EP05-derived check: no full-page MATLAB screenshot may be inserted as a tiny image inside another page. MATLAB assets should be local proof objects such as formulas, rotating vectors, value bars, or source-backed plots.
- No blank waiting pages, production-side labels, shifted formula highlights, detached arrows, or text/formula jitter are accepted in keyframes.
...
- High-risk terms must pass pronunciation normalization before generation. Examples include naked `Q`, `K`, `Q/K`, `KV cache`, polyphonic Chinese phrases such as `长上下文`, and raw formula notation.
- If segmented TTS is used, critical segment audio must be newer than the current manifest before merge or final render.
...
- 每条字幕建议 1.2s-4.5s。
- 字幕不遮挡核心图。
- Dynamic subtitles must be burned into the final review MP4, follow voice timing, and avoid production-side terms such as `读作`, `Hook`, `视觉焦点`, `教学边界`, `QA`, or local file names.
- Captions should match the meaning and timing of the口播 and may add short professional-term annotations; they cannot replace a detailed口播 with a vague summary.
...
- `qa_report.json` 通过。
- `auto_publish = false`。
- Final video review must include audio/video stream probe, final MP4 keyframes, pronunciation/freshness reports when TTS was regenerated, SFX mix status when sound cues are required, and an explicit verified/not-verified closeout.
...
- `docs/visual_system/EP05_ROPE_POSTMORTEM.md`：EP05 RoPE 的技术语义、证据边界、广告学画面、MATLAB、字幕、TTS、音效、布局和 QA 复盘。
- `docs/superpowers/specs/2026-06-20-ep05-stage-gate-map.md`：把 EP05 复盘拆分到每个制作环节的阻断门禁和验证证据。
