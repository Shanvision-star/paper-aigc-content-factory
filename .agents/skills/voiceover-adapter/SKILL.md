...
- Apply EP05-derived high-risk term handling when relevant: do not send naked `Q`, `K`, `V`, `Q/K`, `KV cache`, raw `mθ_i`, `nθ_i`, `δ_i`, or polyphonic phrases such as `长上下文` directly to TTS without a reviewed `spoken_text` form.
- Record a text hash or manifest timestamp that downstream freshness gates can compare against generated segment wav files.
...
- Do not modify `caption_text` or `visual_text` to hide a TTS pronunciation workaround; pronunciation fixes belong in `spoken_text`.
