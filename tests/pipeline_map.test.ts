...
describe("pipeline map", () => {
  it("shows each P0 stage input, output, and quality state", () => {
    ...
    expect(stageById.quality_gate.blocking_items).toHaveLength(6);
    ...
    expect(stageById.video_draft.command).toBe("npm run video:hyperframes-draft");
    expect(stageById.video_draft.outputs.map((item) => item.path)).toContain("renders/hyperframes/ep01_draft.html");
    expect(stageById.video_draft.outputs.map((item) => item.path)).not.toContain("renders/douyin_zh_1080x1920_draft.mp4");
    expect(stageById.video_render).toBeUndefined();
...
describe("Dagu workflow", () => {
  it("connects the existing npm scripts in dependency order", () => {
    ...
    expect(steps.video_draft.depends).toBe("captions");
